...
from features import ImageFeatures, extract_query_region_features, extract_sift_features, global_descriptor_from_path
...
class PipelineConfig:
    ...
    def resolved(self) -> "PipelineConfig":
        if self.coords_path is None:
            self.coords_path = find_coords_path(self.project_dir)
        return self
...
def load_localization_context(config: Optional[PipelineConfig] = None, verbose: bool = True) -> LocalizationContext:
    ...
    for name, meta in sorted(metadata.items(), key=lambda item: natural_sort_key(item[0])):
        ...
        if features is None and config.allow_primary_feature_fallback:
            computed = extract_sift_features(meta.image_path, nfeatures=config.sift_nfeatures)
    ...
    if verbose:
        ...
        print(f"  coords: {config.coords_path or 'not found, inferred from filenames'}")
...
def _compute_global_scores_if_needed(context: LocalizationContext, query_path: Path) -> tuple[Optional[Dict[str, float]], bool]:
    names = list(context.references.keys())

    query_vec = global_descriptor_from_path(query_path)
    scores: Dict[str, float] = {}
    for name, entry in context.references.items():
        ref_vec = global_descriptor_from_path(entry.meta.image_path)
        size = min(query_vec.size, ref_vec.size)
        scores[name] = float(np.dot(query_vec[:size], ref_vec[:size]))
    shortlist_used = len(names) > context.config.ranking.large_dataset_threshold
    return scores, shortlist_used
...
def run_primary_localization(
    ...
    global_scores, shortlist_used = _compute_global_scores_if_needed(context, query_path)
...
def localize_query_image(
    ...
    if verbose:
        print(f"Query features: {len(query_features.descriptors)}")
        print("Primary reference localization:")

    primary = run_primary_localization(context, query_path, query_features, verbose=verbose)
    best: CandidateResult = primary["best"]
    pnp = try_optional_pnp(context, query_features, primary)
